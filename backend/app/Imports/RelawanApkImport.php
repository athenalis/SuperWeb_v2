...
class RelawanApkImport implements ToCollection, WithHeadingRow
{
    ...
    {
        $koor = DB::table('apk_koordinators')
            ->where('id', $this->koorApkId)
            ->whereNull('deleted_at')
            ->first();

        if (!$koor) throw new \Exception('Koordinator APK tidak valid');
        ...
        foreach ($rows as $i => $row) {
            ...
            if ($existing && !$existing->trashed()) {
                ...
                if ((int)$existing->is_apk === 1) {
                    $this->failedRows[] = [
                        'row' => $rowNumber,
                        'nama' => $data['nama'],
                        'errors' => ['NIK sudah terdaftar sebagai relawan APK'],
                    ];
                    continue;
                }
