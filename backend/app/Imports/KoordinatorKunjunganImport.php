...
class KoordinatorKunjunganImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            $existsHp = DB::table($this->tableKunjungan)->where('no_hp', $hp)->whereNull('deleted_at')->exists()
                || DB::table('apk_koordinators')->where('no_hp', $hp)->whereNull('deleted_at')->exists()
            ...
            if ($existsNik) {
                $this->failedRows[] = [
                    ...
                    'errors' => ['NIK sudah terdaftar (koordinator/relawan)']
                ];
                continue;
            }
            ...
            if ($existsHp) {
                $this->failedRows[] = [
                    'row' => $rowNumber,
                    'nama' => $row[$headers['nama']],
                    'errors' => ['No HP sudah terdaftar (koordinator/relawan)']
                ];
                continue;
            }
            ...
            if (($existingCount + $sessionCount) >= 2) {
                $this->failedRows[] = [
                    'row' => $rowNumber,
                    'nama' => $row[$headers['nama']],
                    'errors' => ["Desa/Kelurahan '{$village->village}' sudah memiliki " . ($existingCount + $sessionCount) . " koordinator kunjungan (Maksimal 2)."]
                ];
                continue;
            }
            ...
            try {
                DB::transaction(function () use ($row, $headers, $province, $city, $district, $village, $nik, $hp) {
                    ...
                    DB::table($this->tableKunjungan)->insert([
                        'user_id'        => $user->id,
