...
export default function APKIndex() {
    ...
    const isAdminPaslon = role === "admin_paslon";
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const { data } = await api.delete(`/apk/items/${itemToDelete.id}`);
            if (data?.status) {
                toast.success("Item berhasil dihapus");
                // Refresh items
                const res = await api.get("/apk/items");
                setItems((res?.data?.data || []).map(mapItemBEtoFE));
        ...
        <div className="min-h-screen bg-slate-50 border border-slate-200 rounded-xl p-8 space-y-6">
            ...
            {showHistoryModal && historyItem && (
                <HistoryModal
                    ...
                    mapHistoryBEtoFE={mapHistoryBEtoFE} // pass helper
                />
            )}
...
function DeleteConfirmationModal({ item, isOpen, loading, onClose, onConfirm }) {
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md border border-slate-200 overflow-hidden">
                <div className="p-6 text-center">
                    <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4">
                        <Icon icon="mdi:alert-circle-outline" className="text-red-500" width={40} />
                    </div>
                    <h3 className="text-xl font-bold text-slate-800 mb-2">Hapus Item?</h3>
                    <p className="text-slate-500 mb-6">
                        Apakah Anda yakin ingin menghapus <b>{item.nama}</b>? <br />
                        Tindakan ini juga akan menghapus <b>riwayat stok</b> terkait dan tidak dapat dibatalkan.
                    </p>
                    ...
                    <div className="flex gap-3 justify-center">
                        <button
                            onClick={onClose}
                            disabled={loading}
                            className="px-5 py-2.5 bg-white border border-slate-300 rounded-lg font-medium text-slate-700 hover:bg-slate-50 transition disabled:opacity-50"
                        >
                            Batal
                        </button>
                        <button
                            onClick={onConfirm}
                            disabled={loading}
                            className="px-5 py-2.5 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition shadow-sm flex items-center gap-2 disabled:opacity-50"
                        >
                            {loading ? (
                                <>
                                    <Icon icon="mdi:loading" className="animate-spin" /> Menghapus...
                                </>
                            ) : (
                                "Ya, Hapus"
                            )}
...
function HistoryModal({ item, onClose, mapHistoryBEtoFE }) {
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            setLoading(true);
            try {
                const res = await api.get(`/apk/stock/history/${item.id}`, {
                    params: { per_page: 5, page }
                });
                if (res.data?.status) {
                    // map backend data to frontend format
                    const rows = (res.data.data.data || []).map(mapHistoryBEtoFE);
                    setData(rows);
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl border border-slate-200 flex flex-col max-h-[90vh]">
                <div className="p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50 rounded-t-xl">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">Item: <span className="font-semibold text-slate-700">{item.nama}</span></p>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    <table className="w-full text-left text-xs sm:text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                <tr><td colSpan={5} className="p-8 text-center text-slate-500">Belum ada riwayat.</td></tr>
                            ) : (
                                data.map((h) => {
                                    const isMasuk = h.tipe === 'masuk';
                                    const isKeluar = h.tipe === 'keluar';
                                    ...
                                    return (
                                        <tr key={h.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="px-5 py-3 text-slate-500 italic truncate max-w-[150px]">
                                                {h.tujuan}
                                            </td>
                                        </tr>
                                    )
                ...
                <div className="p-4 border-t border-slate-200 bg-slate-50 rounded-b-xl flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="px-3 py-1.5 bg-white border border-slate-300 rounded text-xs font-medium text-slate-600 disabled:opacity-50 hover:bg-slate-50"
                        >
                            Prev
                        </button>
                        <button
