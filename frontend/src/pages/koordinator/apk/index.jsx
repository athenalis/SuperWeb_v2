...
export default function KoordinatorApk() {
  ...
  // ================= FETCH =================
  const fetchKoordinatorApk = async () => {
    const res = await api.get("/koordinator-apk", {
      params: {
  ...
  const {
    ...
    queryKey: ["koordinator-apk", activeFilters],
    queryFn: fetchKoordinatorApk,
  });
  ...
  // Template download - menggunakan endpoint template yang sudah ada di backend
  const downloadTemplate = async () => {
    try {
      const res = await api.get("/koordinator-apk/template", {
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "template_koordinator_apk.xlsx");
  ...
  const exportAllKoordinatorApk = async () => {
    ...
    const toastId = "export-koordinator-apk";
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/koordinator-apk/${id}`),

    onMutate: () => {
      setDeleteError(null);
      toast.loading("Menghapus koordinator...", { id: "delete-koor-apk" });
    },

    onSuccess: () => {
      queryClient.invalidateQueries(["koordinator-apk"]);
      toast.success("Koordinator berhasil dihapus", { id: "delete-koor-apk" });
      setDeleteTarget(null);
    },

    onError: (err) => {
      toast.dismiss("delete-koor-apk");
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-slate-400">
              Cari data berdasarkan Nama, NIK, atau Wilayah
            </div>
          </div>
        </div>
            ...
            paginatedData.map((item) => (
              ...
              >
                ...
                <div className="pt-3 border-t flex items-center gap-2">
                  ...
                  <button
                    onClick={() => navigate(`/koordinator/apk/${item.id}`)}
        ...
        <table className="w-full text-base hidden md:table">
          <thead className="bg-slate-100">
            <tr>
              ...
              <th className="px-5 py-4 text-center hidden md:table-cell">Status</th>
            ...
            <div className="relative w-full max-w-sm sm:max-w-md bg-white rounded-2xl shadow-2xl p-6 z-10">
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3">
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 rounded-lg border text-slate-600 hover:bg-slate-100 transition"
                >
                  Batal
                </button>

                <button
                  onClick={exportAllKoordinatorApk}
                  disabled={exporting}
                  className={`w-full sm:w-auto px-5 py-2 rounded-lg bg-blue-900 text-white hover:bg-blue-800 transition ${exporting ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                >
